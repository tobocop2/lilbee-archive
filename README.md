...
| How                   | Command                                                                                  | Notes                                                                                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Homebrew**          | `brew tap tobocop2/lilbee && brew install lilbee`                                        | macOS (Apple silicon and Intel) / Linux x86_64. Bundled build; clears the macOS quarantine flag for you.                                                                                                                                  |
...
| **Flatpak**           | `flatpak remote-add --if-not-exists lilbee https://tobocop2.github.io/flatpak-lilbee/lilbee.flatpakrepo && flatpak install lilbee io.github.tobocop2.lilbee` | Linux x86_64, any distro with flatpak. Needs the [Flathub remote](https://flathub.org/setup) for the runtime. Run with `flatpak run io.github.tobocop2.lilbee`, or make it a plain command: `alias lilbee='flatpak run io.github.tobocop2.lilbee'` in your shell rc (swap in `.cuda` / `.rocm` / `.compat` for a vendor build). `flatpak update` picks up new releases. Data lives under `~/.var/app/io.github.tobocop2.lilbee/`. Prefer one file? Grab `lilbee.flatpakref` from the [release](https://github.com/tobocop2/lilbee/releases/latest) and `flatpak install ./lilbee.flatpakref`. |
...
| **Scoop**             | `scoop bucket add lilbee https://github.com/tobocop2/lilbee; scoop install lilbee` | Windows x86_64. Reads your NVIDIA driver version and installs the matching CUDA build (555.85+ gets cu125, 551.61+ gets cu124), otherwise the Vulkan build. It says which one it picked. `scoop update lilbee` upgrades. |
...
| **Flatpak**  | `flatpak install lilbee io.github.tobocop2.lilbee.cuda`                                                                                                                      |
...
| Capability | lilbee wiki | [STORM](https://github.com/stanford-oval/storm) | [GraphRAG](https://github.com/microsoft/graphrag) | [DeepWiki-Open](https://github.com/AsyncFuncAI/deepwiki-open) |
|---|---|---|---|---|
