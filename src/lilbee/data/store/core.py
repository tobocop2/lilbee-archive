...
class Store:
        ...
        with self._write_lock():
            ...
            try:
                if _has_fts_index(table):
                    ...
                    table.create_index(
                        _CHUNK_COLUMN, config=FTS(with_position=False), replace=False
                    )
                    self._fts_ready = True
    ...
    def _ensure_title_fts_unlocked(self, table: lancedb.table.Table) -> None:
        ...
        try:
            ...
            table.create_index(_TITLE_COLUMN, config=FTS(with_position=False), replace=False)
    ...
    def _rebuild_fts_positionless(self, table: lancedb.table.Table) -> None:
        ...
        try:
            table.create_index(_CHUNK_COLUMN, config=FTS(with_position=False), replace=True)
            if self._config.title_search and _TITLE_COLUMN in table.schema.names:
                table.create_index(_TITLE_COLUMN, config=FTS(with_position=False), replace=True)
    ...
    def _ensure_scalar_index_on(
        ...
        scalar_configs = {"BTREE": BTree, "BITMAP": Bitmap}
        ...
        for column, index_type in columns:
            if column not in names or _has_scalar_index(table, column):
                continue
            try:
                table.create_index(column, config=scalar_configs[index_type](), replace=False)
