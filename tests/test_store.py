...
class TestEnsureFtsIndex:
    ...
    def test_positional_index_overflow_rebuilds_positionless(self, store):
        ...
        assert any(
            ...
            and c.kwargs.get("config").with_position is False
    ...
    def test_overflow_rebuild_also_rebuilds_the_title_index_when_enabled(self, store, test_config):
        ...
        rebuilt = {
            c.args[0]
            for c in rebuild.call_args_list
            if c.kwargs.get("replace") is True and c.kwargs.get("config").with_position is False
        }
    ...
    def test_overflow_rebuild_failure_is_swallowed(self, store):
        ...
        with (
            mock.patch.object(type(table), "optimize", side_effect=overflow),
            mock.patch.object(
                type(table), "create_index", side_effect=RuntimeError("rebuild boom")
            ),
        ):
    ...
    def test_first_call_creates_chunk_index_only_when_title_search_off(self, store):
        """With title_search off (default), only the chunk index is built via
        create_index(config=FTS()) with replace=False; the title index is not
        created on a store that never queries it."""
        ...
        config = create_spy.call_args_list[0].kwargs.get("config")
        assert isinstance(config, FTS)
        assert config.with_position is False
        assert create_spy.call_args_list[0].kwargs.get("replace") is False
    ...
    def test_first_call_creates_both_indexes_when_title_search_on(self, store, test_config):
        ...
        # matching.
        assert all(
            isinstance(c.kwargs.get("config"), FTS)
            and c.kwargs.get("config").with_position is False
...
class TestEnsureScalarIndexes:
    ...
    def test_creates_btree_on_source_and_bitmap_on_chunk_type(self, store):
        ...
        with mock.patch.object(type(table), "create_index") as spy:
            store.ensure_scalar_indexes()
        assert [c.args[0] for c in spy.call_args_list] == ["source", "chunk_type"]
        configs = {c.args[0]: c.kwargs.get("config") for c in spy.call_args_list}
        assert isinstance(configs["source"], BTree)
        assert isinstance(configs["chunk_type"], Bitmap)
    ...
    def test_handles_exception_gracefully(self, store):
        ...
        table = store.open_table("chunks")
        assert table is not None
        with mock.patch.object(type(table), "create_index", side_effect=RuntimeError("boom")):
    ...
    def test_one_column_failure_does_not_skip_the_other(self, store):
        ...
        with mock.patch.object(type(table), "create_index", _record):
            store.ensure_scalar_indexes()
