...
# Regenerate the wheel's pin module so the shipped engine identity always
# reflects these sourced versions (sharing between lilbee processes keys on it).
cat > "${script_dir}/../../packaging/engine-wheel/lilbee_engine/pins.py" <<PINS
"""Engine build identity; regenerated by tools/wheel-build/build_llama_server.sh."""

ENGINE_PIN = "llama-cpp-${ENGINE_LLAMA_CPP_VERSION}+swap-${ENGINE_LLAMA_SWAP_VERSION}+gguf-${ENGINE_GGUF_PARSER_REF}"
PINS
